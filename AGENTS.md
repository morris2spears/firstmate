...
- `cipher-hook` - load after reconciling a genuine `needs-decision`, for a checks-green iinvy or iinvy-storefront PR, or on an authenticated `cipher-comment` or `cipher-retry` check notification.
