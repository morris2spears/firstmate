...
#          prints an FMTG line.
#          Carbon peer relay is OPTIONAL and inert unless config/peer-relay-carbon
#          exists. Bootstrap writes and hash-registers its pending-request check;
#          success is a BOOTSTRAP_INFO fact and a refusal is an FMPEER diagnostic.
...
#          x_mode_setup, tg_mode_setup, peer_relay_setup, fleet_sync) while still printing every read-only detect line
#          above; the TANGLE line switches to advisory-only wording with no
#          checkout command. Used by
#          fm-session-start.sh's read-only path when another live session holds
#          the fleet lock, so a second concurrent session never race-mutates
#          PR-check artifacts, secondmate homes, X-mode, Telegram-mode, or peer-relay
...
# shellcheck source=bin/fm-peer-relay-lib.sh disable=SC1091
...
# Carbon peer relay (opt-in): a generated custom check polls only durable ids.
# The generic custom-check trust record binds the exact generated bytes, so the
# watcher uses its existing authenticated snapshot path without a new executor.
peer_relay_setup() {
  local check trust body
  check="$STATE/$FMPEER_CHECK_ID.check.sh"
  trust="$STATE/$FMPEER_CHECK_ID.check-trust"

  if ! fmpeer_enabled "$FM_HOME"; then
    if x_mode_artifact_present "$check" || x_mode_artifact_present "$trust"; then
      if ! x_mode_remove_artifact "$check" || ! x_mode_remove_artifact "$trust"; then
        echo 'FMPEER: peer relay unavailable - stale poll artifacts could not be removed'
        return 0
      fi
      echo 'BOOTSTRAP_INFO: carbon peer relay off - removed pending-request poll'
    fi
    return 0
  fi

  mkdir -p "$STATE" 2>/dev/null \
    || { echo 'FMPEER: peer relay unavailable - state directory could not be prepared'; return 0; }
  body=$(fmpeer_poll_check_content "$FM_HOME" "$FM_ROOT")
  x_mode_write_if_changed "$check" "$body" 700 \
    || { echo 'FMPEER: peer relay unavailable - pending-request poll could not be written'; return 0; }
  if ! FM_HOME="$FM_HOME" FM_STATE_OVERRIDE="$STATE" \
    "$SCRIPT_DIR/fm-check-register.sh" "$FMPEER_CHECK_ID" >/dev/null; then
    x_mode_remove_artifact "$check" || true
    x_mode_remove_artifact "$trust" || true
    echo 'FMPEER: peer relay unavailable - pending-request poll could not be authenticated'
    return 0
  fi
  echo 'BOOTSTRAP_INFO: carbon peer relay on - pending-request poll armed'
}
