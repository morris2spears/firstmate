...
family_for_basename() {
  case "$1" in
    ...
    fm-operational-input.test.sh|fm-pi-decision-nudge.test.sh|fm-pi-primary-types.test.sh|\
    ...
    fm-herdr-session-cleanup-e2e.test.sh|fm-backend-herdr-project-spaces-e2e.test.sh|\
    ...
    fm-send-secondmate-marker-herdr-e2e.test.sh)
      printf '%s\n' live-harness-optin
      ;;
    fm-backend-herdr.test.sh|fm-backend-herdr-project-spaces.test.sh|\
    ...
    fm-teardown-endpoint-safety.test.sh)
      printf '%s\n' backend-dispatch
      ;;
    fm-cipher-hook.test.sh|fm-pr-check-security.test.sh|fm-pr-merge.test.sh|fm-review-diff.test.sh|\
    fm-teardown.test.sh|fm-tg-mode.test.sh|fm-x-mode.test.sh)
      printf '%s\n' pr-forge
      ;;
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-gate-refuse*|bin/fm-lock*)
      printf '%s\n' session-bootstrap
      ;;
    bin/fm-pr-*|bin/fm-cipher-*|bin/fm_cipher_repositories.py|\
