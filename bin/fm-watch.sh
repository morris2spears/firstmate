...
while :; do
  ...
  if [ "$(age_of "$STATE/.last-check")" -ge "$CHECK_INTERVAL" ]; then
    ...
    for c in "$STATE"/*.check.sh; do
      ...
      elif [ "$(basename "$c")" = tg-watch.check.sh ]; then
        # Strict byte-validated sibling of the X arm above: same identity
        # check, same trusted-repository-script dispatch, same rejection path.
