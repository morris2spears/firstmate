...
| `fm-decision-hold.sh`    | Create, verify, complete, and close durable captain-held decisions by routed or terminal disposition |
...
| `fm-decision-nudge.sh`   | Claude Code and Pi primary hook entrypoint sending one content-free Telegram nudge when a decision the captain owns waits unanswered (docs/configuration.md "Captain-attention nudge") |
...
| `fm-pr-lib.sh`           | Own canonical task and PR validation, the literal Cipher-gated repository list, plus private atomic PR-poll publication and identity-bound retirement |
...
| `fm-cipher-hook.sh`      | Deliver authenticated decision and iinvy exact-head events, retry transiently held deliveries after gateway recovery, durably close a keyed decision answered by an authenticated Cipher comment, and expose Cipher's guarded merge entrypoint |
...
| `fm-away-ledger-lib.sh`  | Single owner of the away escalation batch ledger, its private digests, and their retirement |
| `fm-tg-lib.sh`           | Shared Telegram-mode opt-in, pending-note, shim-identity, task-link, and outbound `tg` client helpers |
