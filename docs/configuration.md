...
Only outcomes that prove nothing left the box - a missing client, a watchdog that could not start, or a failure to write the ledger, evidence, or outbound spool - give a reservation back. A watchdog timeout, or a client that ran and exited non-zero (the real client cannot distinguish Telegram rejecting the message from a lost response to one it already accepted), keeps the reservation and reports `uncertain`, and because acceptance may only extend a contiguous evidence-proven prefix, that batch sends nothing further to the phone and finishes through the visible in-session fallback.
...
Every send goes through the ledger by construction: `telegram_away_deliver` requires the batch id and the current reserved/total/accounted counts, refuses when they do not match the ledger, and mints no delivery id of its own.
