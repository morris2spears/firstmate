...
`FM_TG_AWAY_RETRY_SECS` (default: one `FM_ESCALATE_BATCH_SECS` window) is that schedule, so a persistent local failure retries on a bounded cadence instead of re-attempting and minting another evidence record on every housekeeping tick; a deferred flush says so in session and contacts nobody.
...
FM_TG_AWAY_RETRY_SECS=            # seconds a retired proven-local away Telegram attempt waits before the same lines may be offered to the phone again; defaults to one FM_ESCALATE_BATCH_SECS window, invalid values use 90
