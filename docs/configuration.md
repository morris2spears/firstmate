...
`bin/fm-away-ledger-lib.sh` owns one ledger per batch in `state/.subsuper-escalations.since` - the batch identity plus its `reserved`, `confirmed`, and `accounted` line counts and its attempt ordinal - and the away daemon, away start, and away return all query and transition that one owner instead of keeping counters of their own.
...
Only outcomes that prove nothing left the box - a missing client, a watchdog that could not start, or a failure to write the ledger, evidence, or outbound spool - retire the attempt: the ledger gives the reservation back and advances the batch's attempt ordinal in the same atomic write, so the very same lines are offered to the phone again under a fresh delivery id on the next flush while the retired attempt's evidence record stays durable and auditable. A watchdog timeout, or a client that ran and exited non-zero (the real client cannot distinguish Telegram rejecting the message from a lost response to one it already accepted), keeps the reservation and reports `uncertain`, and because acceptance may only extend a contiguous evidence-proven prefix, that batch sends nothing further to the phone and finishes through the visible in-session fallback.
