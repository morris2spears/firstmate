...
Claude's `--claude` mode also treats `state/x-watch.check.sh`, `state/tg-watch.check.sh`, and `state/peer-relay-watch.check.sh` as supervision need, so X-mode relay polling, Telegram-mode inbox polling, and carbon peer-relay request polling remain guarded without an in-flight task.
...
- A valid secondmate home is in scope; an idle secondmate endpoint with no X-mode relay poll, no Telegram-mode inbox poll, and no carbon peer-relay poll remains healthy because it has no supervision need.
