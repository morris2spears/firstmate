...
Claude's `--claude` mode also treats `state/x-watch.check.sh` and `state/tg-watch.check.sh` as supervision need, so X-mode relay polling and Telegram-mode inbox polling remain guarded without an in-flight task.
...
- Claude registers two turn-end `Stop` hooks in `.claude/settings.json`, both anchored through `CLAUDE_PROJECT_DIR`: `bin/fm-turnend-guard.sh --claude`, and `bin/fm-claude-stop-autoarm.sh` with `asyncRewake: true` and `timeout: 28800`.
  A third unrelated `Stop` entry disarms the captain-attention nudge (`docs/configuration.md` "Captain-attention nudge") and never affects continuation.
...
- A valid secondmate home is in scope; an idle secondmate endpoint with no X-mode relay poll and no Telegram-mode inbox poll remains healthy because it has no supervision need.
