...
This is the same predicate `bin/fm-sessionstart-nudge.sh` and `bin/fm-turnend-guard.sh` use, so the tracked primary-scoped hooks cannot drift apart.
...
`bin/fm-supervision-lib.sh` also recognizes an X-mode relay poll or a Telegram-mode inbox poll as supervision need, but unaccounted primary work still contributes nothing to that predicate.
Without one of those independent mode needs, unaccounted primary work therefore reads as idle rather than suspicious.
