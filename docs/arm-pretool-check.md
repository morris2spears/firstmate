...
The allowed cadence-env paths are `config/x-mode.env` and `config/tg-mode.env`, each also accepted with a `./` prefix or as an absolute path that normalizes to that file under `<active-firstmate-home>`.
