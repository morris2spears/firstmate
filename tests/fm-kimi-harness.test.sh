...
test_existing_launch_templates_are_byte_pinned() {
  ...
  assert_source_line "        printf '%s%s' \"\$harness\" ' __MODELFLAG____EFFORTFLAG__-e __PITURNEND__ -e __PIWATCH__ -e __PINUDGE__ \"\$(__OPINPUT__ encode launch-brief < __BRIEF__)\"'"
  assert_source_line "        printf '%s%s' \"\$harness\" ' __MODELFLAG____EFFORTFLAG__-e __PIEXT__ __PICALMFLAG__\"\$(__OPINPUT__ encode launch-brief < __BRIEF__)\"'"
  ...
  pass "fm-spawn: non-Pi and secondmate launch templates stay byte-pinned while ordinary Pi includes Calm"
}
